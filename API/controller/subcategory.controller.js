...
export const fetchById = async (req, res) => {
  try {
    ...
    const subcat = await SubCategorySchemaModel.findOne({ _id: id });

    if (!subcat) {
      return res.status(404).json({ status: false, msg: "Subcategory not found" });
    }

    res.status(200).json({ status: true, subcat });
...
export const update = async (req, res) => {
  try {
    ...
    if (req.files && req.files.caticon) {
      ...
      await caticon.mv(uploadpath);
      updateData.subcaticonnm = subcaticonnm;
    }

    await SubCategorySchemaModel.updateOne(
      { _id: id },
      { $set: updateData }
    );

    res.status(200).json({ status: true, msg: "Subcategory updated" });
...
export const deleteSubCategory = async (req, res) => {
  try {
    ...
    await SubCategorySchemaModel.deleteOne({ _id: id });

    res.status(200).json({ status: true, msg: "Subcategory deleted" });
