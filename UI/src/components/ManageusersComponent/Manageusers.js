...
function Manageusers() {
  ...
  const changeuserstatus = (_id, s)=>{
    if (s=="active")
    {
      axios.patch(__userapiurl+"update", {"condition_obj":{"_id":_id}, "content_obj":{"status":1}}
     ).then(()=>{
    ...
    {
      axios.delete(__userapiurl+"delete", {"data":{"_id":_id}}
      ).then (()=>{
...
<h2 style={{"margin-top":100}}>View & Manage User Details</h2>
...
{ users.map((row)=>(
  ...
  <br/>
  <font color="red" onClick={ () =>{ changeuserstatus(row._id, 'delete')}}>Delete</font>
  </td>
  </tr>
)) }
